...
class WebService {
    ...
    func specalPost(url: String, params: Dictionary<String, Any>?, completion:@escaping (JSON?) -> Void, failed: @escaping (String) -> Void)
    {
        webService(url: url, method: .post, params: params,specialFlag: 1, completion: completion, failed: failed)
    }
    ...
    private func webService(url: String, method: Alamofire.HTTPMethod, params: Dictionary<String, Any>?,specialFlag:Int = 0,specialEncoding:ParameterEncoding = URLEncoding.default, completion: @escaping (JSON?) -> Void, failed: @escaping (String) -> Void)
        {
            print(url)
            if(params != nil)
            {
    ...
    private func getHeaders(url urlString: String) -> HTTPHeaders{
       ...
       headers["token"] = "\(AppPreferences.getToken())"
